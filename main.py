...
def generate_one(page:int, save_dir:pathlib.Path):
    ...
    for s_type, aux, base in specified:
        ...
        if s_type == "ngon":
            ...
            d_ngon1 = regular_ngon(diff_ns1) * BASE_SCALE
            dummies_raw.insert(0, (d_ngon1, base_max_dim))
            
            available_ns2 = [n for n in available_ns if n != diff_ns1]
            if available_ns2:
                diff_ns2 = rnd.choice(available_ns2)
                d_ngon2 = regular_ngon(diff_ns2) * BASE_SCALE
                dummies_raw.insert(1, (d_ngon2, base_max_dim))
...
# ──────────────────────────────────
# 4. 追加機能：Google Drive API アップロード
# ──────────────────────────────────
